package cn.rivamed.log.task.xxlJob;
 ...
 */
@Aspect
public class XxlJobTaskAop {

    private static final Logger logger = LoggerFactory.getLogger(XxlJobTaskAop.class);

    private final Tracer tracer;

    public XxlJobTaskAop(Tracer tracer) {
        this.tracer = tracer;
    }
    ...
    public Object around(ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            ...
            stopWatch.stop();
            if (taskEnable) {
                logger.info(LogMessageConstant.LOG_TYPE_XXLJOB_TASK_LOG + String.format(LogTemplateUtil.TASK_SUCCESS_FORMAT, dateStr, method, stopWatch.getTime()));
            }
            return proceed;
        } catch (Throwable ex) {
            String message = ex.getMessage() == null ? ex.getClass().getSimpleName() : ex.getMessage();
            if (taskEnable) {
                logger.error(LogMessageConstant.LOG_TYPE_XXLJOB_TASK_LOG + String.format(LogTemplateUtil.TASK_FAIL_FORMAT, dateStr, method), message);
            }
            throw ex;
        } finally {
            span.finish();
